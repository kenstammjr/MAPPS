...
namespace MAPPS {
    public class ServerVersion {
        ...
        public ServerVersion(int ID)
                : this() {
            using (new Impersonator()) {
                SqlConnection conn = DataSource.Conn();
                string sql = "SELECT * FROM dbo.ServerVersions WHERE ID = @ID";
        ...
        public ServerVersion(string Name)
            ...
            using (new Impersonator()) {
                SqlConnection conn = DataSource.Conn();
                string sql = "SELECT * FROM dbo.ServerVersions WHERE Name = @Name";
                SqlCommand cmd = new SqlCommand(sql, conn);
                cmd.CommandType = CommandType.Text;
        ...
        public bool Insert() {
            ...
            using (new Impersonator()) {
                ...
                try {
                    ...
                    string sql = @"INSERT INTO dbo.ServerVersions 
			                                (Name,
										Description,
										CreatedOn,
										CreatedBy,
        ...
        public bool Update() {
            ...
            using (new Impersonator()) {
                ...
                try {
                    DateTime TimeStamp = DateTime.UtcNow;
                    string sql = @"UPDATE dbo.ServerVersions 
			                        SET Name = @Name,
        ...
        public bool Delete() {
            ...
            using (new Impersonator()) {
                SqlConnection conn = DataSource.Conn();
                try {
                    string sql = "DELETE FROM dbo.ServerVersions WHERE ID = @ID";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.Parameters.AddWithValue("@ID", _ID);
                    if (conn.State != ConnectionState.Open) { conn.Open(); }
                    cmd.ExecuteNonQuery();
        ...
        public static DataSet Items() {
            ...
            using (new Impersonator()) {
                SqlConnection conn = DataSource.Conn();
                try {
                    string sql = "SELECT * FROM dbo.ServerVersions Order By Name";
                    SqlCommand cmd = new SqlCommand(sql, conn);
        ...
        public static bool Exists(string ServerVersion) {
            bool found = false;

            using (new Impersonator()) {
                SqlConnection conn = DataSource.Conn();
                const string sql = @"SELECT Count(ID) AS cnt 
                                    FROM dbo.ServerVersions 
