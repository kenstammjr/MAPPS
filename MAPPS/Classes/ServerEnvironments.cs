...
namespace MAPPS {
    public class ServerEnvironment {
        ...
        public ServerEnvironment(int ID)
                : this() {
            using (new Impersonator()) {
                SqlConnection conn = DataSource.Conn();
                string sql = "SELECT * FROM dbo.ServerEnvironments WHERE ID = @ID";
        ...
        public bool Insert() {
            ...
            using (new Impersonator()) {
                ...
                try {
                    ...
                    string sql = @"INSERT INTO dbo.ServerEnvironments 
			                                (Name,
										Description,
										CreatedOn,
										CreatedBy,
        ...
        public bool Update() {
            ...
            using (new Impersonator()) {
                ...
                try {
                    DateTime TimeStamp = DateTime.UtcNow;
                    string sql = @"UPDATE dbo.ServerEnvironments 
			                        SET Name = @Name,
        ...
        public bool Delete() {
            ...
            using (new Impersonator()) {
                SqlConnection conn = DataSource.Conn();
                try {
                    string sql = "DELETE FROM dbo.ServerEnvironments WHERE ID = @ID";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.Parameters.AddWithValue("@ID", _ID);
                    if (conn.State != ConnectionState.Open) { conn.Open(); }
                    cmd.ExecuteNonQuery();
        ...
        public static DataSet Items() {
            ...
            using (new Impersonator()) {
                SqlConnection conn = DataSource.Conn();
                try {
                    string sql = "SELECT * FROM dbo.ServerEnvironments";
                    SqlCommand cmd = new SqlCommand(sql, conn);
