...
namespace MAPPS.Pages {
    public partial class Tabs : PageBase {
        protected const string PAGE_TITLE = "Tabs";
        protected const string PAGE_DESCRIPTION = "View and manage application tabs";
        public const string PAGE_URL = BASE_PAGES_URL + "Tabs.aspx";
        ...
        protected override void Page_Load(object sender, EventArgs e) {
            try {
                ...
                SetupContribute();
                if (!IsPostBack) {
                    GridSortExpression = "DisplayIndex";
                    Fill();
                }
            } catch (Exception ex) {
        ...
        private void SetupContribute() {
            if (IsManager) {
                jsActionNewItem = string.Format("window.location.replace('{0}/{1}?View=New&ID=0'); return false;", SPContext.Current.Web.Url, TabItem.PAGE_URL);
        ...
        protected override void Fill() {
            try {
                ...
                DataView dv = new DataView(MAPPS.Tab.Items(true).Tables[0]);
            ...
            if (e.CommandName != "Sort" && e.CommandName != "Page") {
                try {
                    ...
                    if (e.CommandName == "MoveDown") {
                        ItemID = int.Parse(e.CommandArgument.ToString());
                        MAPPS.Tab.UpdateDisplayIndex(ItemID, "up");
                        Fill();
                    }
                    // Move Up
                    if (e.CommandName == "MoveUp") {
                        ItemID = int.Parse(e.CommandArgument.ToString());
                        MAPPS.Tab.UpdateDisplayIndex(ItemID, "down");
                        Fill();
                    }
                } catch (Exception ex) {
                    MAPPS.Error.WriteError(ex);
        ...
        protected void lbtnNew_Click(object sender, EventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=New&id={2}", SPContext.Current.Web.Url, Pages.TabItem.PAGE_URL, 0), false);
        }
