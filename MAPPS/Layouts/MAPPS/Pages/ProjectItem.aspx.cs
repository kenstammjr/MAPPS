...
namespace MAPPS.Pages {
    public partial class ProjectItem : PageBase {
        protected const string PAGE_TITLE = "Project Details";
        protected const string PAGE_DESCRIPTION = "View and manage Project details";
        public const string PAGE_URL = BASE_PAGES_URL + "ProjectItem.aspx";
        ...
        protected override void Page_Load(object sender, EventArgs e) {
            try {
                ...
                jsActionCancel = string.Format("rbnCancelClick('{0}/{1}?filter={2}'); return false;", SPContext.Current.Web.Url, Pages.Projects.PAGE_URL, Filter);
                itemRedirect = string.Format("{0}/{1}", SPContext.Current.Web.Url, Pages.Projects.PAGE_URL);
                newItemRedirect = string.Format("{0}/{1}?View=New&id={2}", SPContext.Current.Web.Url, Pages.ProjectItem.PAGE_URL, 0);
        ...
        public virtual string GetItemsURL(bool AsLink) {
            ...
            try {
                string url = string.Format("{0}/{1}?filter={2}", SPContext.Current.Web.Url, Pages.Projects.PAGE_URL, Filter);
                s = (AsLink) ? string.Format("<a href=\"{0}\" class=\"mapps-breadcrumb\">Projects</a>", url) : url;
            } catch (Exception ex) {
                MAPPS.Error.WriteError(ex);
            }
            return s;
        ...
        private void SetupContribute() {
            ...
            if (IsManager) {
                item = (ItemID == 0) ? new MAPPS.Project() : new MAPPS.Project(ItemID);
            } else {
                        ...
                        case RIBBON_POSTBACK_SAVE_EVENT:
                            if (SaveItem())
                                if (SPContext.Current.IsPopUI)
                                    ...
                                    Response.Redirect(string.Format("{0}/{1}?filter={2}", SPContext.Current.Web.Url, Pages.Projects.PAGE_URL, Filter), false);
                            break;
        ...
        private bool SaveItem() {
            ...
            try {
                ...
                item = (isUpdate) ? new MAPPS.Project(ItemID) : new MAPPS.Project();
        ...
        private bool DeleteItem() {
            bool success = false;
            try {
                MAPPS.Project item = new MAPPS.Project(ItemID);
                if (item.Delete()) {
                    success = true;
                    ItemID = 0;
                } else {
        ...
        protected void ibtnRibbonNew_Click(object sender, System.Web.UI.ImageClickEventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=New&id={2}&portid={2}&IsDlg=1&Filter={3}", SPContext.Current.Web.Url, Pages.ProjectItem.PAGE_URL, 0, Filter), false);
        }
        ...
        protected void ibtnRibbonView_Click(object sender, System.Web.UI.ImageClickEventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=View&id={2}&portid={2}&IsDlg=1&Filter={3}", SPContext.Current.Web.Url, Pages.ProjectItem.PAGE_URL, ItemID, Filter), false);
        }
        ...
        protected void ibtnRibbonEdit_Click(object sender, System.Web.UI.ImageClickEventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=Edit&id={2}&portid={2}&IsDlg=1&Filter={3}", SPContext.Current.Web.Url, Pages.ProjectItem.PAGE_URL, ItemID, Filter), false);
