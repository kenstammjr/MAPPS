...
namespace MAPPS.Pages {
    public partial class ServerVersionItem : PageBase {
        protected const string PAGE_TITLE = "Server Version Details";
        protected const string PAGE_DESCRIPTION = "View and manage server version details";
        public const string PAGE_URL = BASE_PAGES_URL + "ServerVersionItem.aspx";

        protected string jsActionCancel;
        private string itemRedirect;
        private string newItemRedirect;

        private ServerVersion item;
        ...
        protected override void Page_Load(object sender, EventArgs e) {
            try {
                ...
                jsActionCancel = string.Format("rbnCancelClick('{0}/{1}'); return false;", SPContext.Current.Web.Url, Pages.ServerVersions.PAGE_URL);
                itemRedirect = string.Format("{0}/{1}", SPContext.Current.Web.Url, Pages.ServerVersions.PAGE_URL);
                newItemRedirect = string.Format("{0}/{1}?View=New&id={2}", SPContext.Current.Web.Url, Pages.ServerVersionItem.PAGE_URL, 0);
        ...
        public virtual string GetItemsURL(bool AsLink) {
            ...
            try {
                ...
                s = (AsLink) ? string.Format("<a href=\"{0}\" class=\"mapps-breadcrumb\">Server Version</a>", itemRedirect) : itemRedirect;
            } catch (Exception ex) {
                MAPPS.Error.WriteError(ex);
            }
            return s;
        ...
        private void SetupContribute() {
            ...
            if (IsManager) {
                item = (ItemID == 0) ? new MAPPS.ServerVersion() : new MAPPS.ServerVersion(ItemID);
            } else {
        ...
        private bool SaveItem() {
            ...
            try {
                ...
                item = (isUpdate) ? new MAPPS.ServerVersion(ItemID) : new MAPPS.ServerVersion();
        ...
        private bool DeleteItem() {
            bool success = false;
            try {
                MAPPS.ServerVersion item = new MAPPS.ServerVersion(ItemID);
                if (item.Delete()) {
                    success = true;
                    ItemID = 0;
                } else {
        ...
        protected void lbtnEdit_Click(object sender, EventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=Edit&id={2}", SPContext.Current.Web.Url, Pages.ServerVersionItem.PAGE_URL, ItemID), false);
        }

        protected void lbtnView_Click(object sender, EventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=View&id={2}", SPContext.Current.Web.Url, Pages.ServerVersionItem.PAGE_URL, ItemID), false);
        }
