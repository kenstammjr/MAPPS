...
namespace MAPPS.Pages {
    public partial class Users : PageBase {
        protected const string PAGE_TITLE = "User Catalog";
        protected const string PAGE_DESCRIPTION = "View and manage the user catalog";
        public const string PAGE_URL = BASE_PAGES_URL + "Users.aspx";
        ...
        protected override void Fill() {
            try {
                ...
                if (txtSearch.Text == string.Empty) {
                    ...
                    dv = new DataView(MAPPS.User.Items().Tables[0]);
                } else {
                    gvData.EmptyDataText = Message.EMPTY_LIST_SEARCHED;
                    dv = new DataView(MAPPS.User.Items(txtSearch.Text.Trim()).Tables[0]);
                }
            ...
            try {
                switch (e.Row.RowType) {
                    case DataControlRowType.DataRow:
                        ...
                        LinkButton lbtnLastName = (LinkButton)e.Row.FindControl("lbtnLastName");
                        ...
                        if (txtSearch.Text.Length > 0) {
                            ...
                            lbtnLastName.Text = Common.HighLightText(lbtnLastName.Text, Filter);
                            lblFirstName.Text = Common.HighLightText(lblFirstName.Text, Filter);
                            lblUserName.Text = Common.HighLightText(lblUserName.Text, Filter);
                            lblPreferredName.Text = Common.HighLightText(lblPreferredName.Text, Filter);
                            lblEmail.Text = Common.HighLightText(lblEmail.Text, Filter);
                        }
            ...
            if (e.CommandName != "Sort" && e.CommandName != "Page") {
                try {
                    if (e.CommandName == "ViewItem") {
                        ...
                        Response.Redirect(string.Format("{0}/{1}?View=View&ID={2}&Filter={3}&UserID={2}", SPContext.Current.Web.Url, UserItem.PAGE_URL, ItemID, Filter), false);
                    }
                    if (e.CommandName == "EditItem") {
                        ItemID = int.Parse(e.CommandArgument.ToString());
                        Response.Redirect(string.Format("{0}/{1}?View=Edit&ID={2}&Filter={3}&UserID={2}", SPContext.Current.Web.Url, UserItem.PAGE_URL, ItemID, Filter), false);
                    }
                } catch (Exception ex) {
                    MAPPS.Error.WriteError(ex);
                    if (ShowDebug)
        ...
        protected void lbtnNew_Click(object sender, EventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=New&ID={2}&UserID={2}&Filter={3}", SPContext.Current.Web.Url, Pages.UserItem.PAGE_URL, 0, Filter), false);
        }
