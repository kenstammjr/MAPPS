...
namespace MAPPS.Pages {
    public partial class CertificateItem : PageBase {
        protected const string PAGE_TITLE = "Certificate Details";
        protected const string PAGE_DESCRIPTION = "View and manage certificate details";
        public const string PAGE_URL = BASE_PAGES_URL + "CertificateItem.aspx";
        ...
        protected override void Page_Load(object sender, EventArgs e) {
            try {
                ...
                jsActionCancel = string.Format("rbnCancelClick('{0}/{1}?filter={2}'); return false;", SPContext.Current.Web.Url, Pages.Certificates.PAGE_URL, Filter);
                itemRedirect = string.Format("{0}/{1}", SPContext.Current.Web.Url, Pages.Certificates.PAGE_URL);
                newItemRedirect = string.Format("{0}/{1}?View=New&id={2}", SPContext.Current.Web.Url, Pages.CertificateItem.PAGE_URL, 0);
        ...
        public virtual string GetItemsURL(bool AsLink) {
            ...
            try {
                string url = string.Format("{0}/{1}?filter={2}", SPContext.Current.Web.Url, Pages.Certificates.PAGE_URL, Filter);
                s = (AsLink) ? string.Format("<a href=\"{0}\" class=\"mapps-breadcrumb\">Certificates</a>", url) : url;
            } catch (Exception ex) {
                MAPPS.Error.WriteError(ex);
            }
            return s;
        ...
        private void SetupContribute() {
            ...
            if (IsManager) {
                item = (ItemID == 0) ? new MAPPS.Certificate() : new MAPPS.Certificate(ItemID);
            } else {
                        ...
                        case RIBBON_POSTBACK_SAVE_EVENT:
                            if (SaveItem())
                                if (SPContext.Current.IsPopUI)
                                    ...
                                    Response.Redirect(string.Format("{0}/{1}?filter={2}", SPContext.Current.Web.Url, Pages.Certificates.PAGE_URL, Filter), false);
                            break;
        ...
        protected override void Fill() {
            try {
                ...
                lblExpirationView.Text = item.Expiration.ToString("dd-MMM-yyyy");
                lblExpirationView.Visible = isView;
                dtcExpiration.Visible = !isView;
                try { dtcExpiration.SelectedDate = item.Expiration; } catch { }
        ...
        private bool SaveItem() {
            ...
            try {
                ...
                item = (isUpdate) ? new MAPPS.Certificate(ItemID) : new MAPPS.Certificate();
        ...
        private bool DeleteItem() {
            bool success = false;
            try {
                MAPPS.Certificate item = new MAPPS.Certificate(ItemID);
                if (item.Delete()) {
                    success = true;
                    ItemID = 0;
                } else {
        ...
        protected void ibtnRibbonNew_Click(object sender, System.Web.UI.ImageClickEventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=New&id={2}&certificateid={2}&IsDlg=1&Filter={3}", SPContext.Current.Web.Url, Pages.CertificateItem.PAGE_URL, 0, Filter), false);
        }
        ...
        protected void ibtnRibbonView_Click(object sender, System.Web.UI.ImageClickEventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=View&id={2}&certificateid={2}&IsDlg=1&Filter={3}", SPContext.Current.Web.Url, Pages.CertificateItem.PAGE_URL, ItemID, Filter), false);
        }
        ...
        protected void ibtnRibbonEdit_Click(object sender, System.Web.UI.ImageClickEventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=Edit&id={2}&certificateid={2}&IsDlg=1&Filter={3}", SPContext.Current.Web.Url, Pages.CertificateItem.PAGE_URL, ItemID, Filter), false);
        ...
        //protected void dtcExpiration_DateChanged(object sender, EventArgs e) {
        //    txtExpiration.Text = dtcExpiration.SelectedDate.ToString("dd-MMM-yyyy");
        //}
    }
}
