...
namespace MAPPS.Pages {
    public partial class ServerTypes : PageBase {
        protected const string PAGE_TITLE = "Server Types";
        protected const string PAGE_DESCRIPTION = "View and manage server types";
        public const string PAGE_URL = BASE_PAGES_URL + "ServerTypes.aspx";
        ...
        private void SetupContribute() {
            if (IsManager) {
                jsActionNewItem = string.Format("window.location.replace('{0}/{1}?View=New&ID=0'); return false;", SPContext.Current.Web.Url, ServerTypeItem.PAGE_URL);
        ...
        protected override void Fill() {
            try {
                ...
                DataView dv = new DataView(MAPPS.ServerType.Items().Tables[0]);
            ...
            if (e.CommandName != "Sort" && e.CommandName != "Page") {
                try {
                    if (e.CommandName == "ViewItem") {
                        ...
                        Response.Redirect(string.Format("{0}/{1}?View=View&ID={2}", SPContext.Current.Web.Url, ServerTypeItem.PAGE_URL, ItemID), false);
                    }
                    if (e.CommandName == "EditItem") {
                        ItemID = int.Parse(e.CommandArgument.ToString());
                        Response.Redirect(string.Format("{0}/{1}?View=Edit&ID={2}", SPContext.Current.Web.Url, ServerTypeItem.PAGE_URL, ItemID), false);
                    }
                } catch (Exception ex) {
                    MAPPS.Error.WriteError(ex);
                    if (ShowDebug)
        ...
        protected void lbtnNew_Click(object sender, EventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=New&id={2}", SPContext.Current.Web.Url, Pages.ServerTypeItem.PAGE_URL, 0), false);
        }
