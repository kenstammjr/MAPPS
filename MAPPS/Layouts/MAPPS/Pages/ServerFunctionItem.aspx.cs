...
namespace MAPPS.Pages {
    public partial class ServerFunctionItem : PageBase {
        protected const string PAGE_TITLE = "Server Function Details";
        protected const string PAGE_DESCRIPTION = "View and manage server function details";
        public const string PAGE_URL = BASE_PAGES_URL + "ServerFunctionItem.aspx";

        protected string jsActionCancel;
        private string itemRedirect;
        private string newItemRedirect;

        private ServerFunction item;
        ...
        protected override void Page_Load(object sender, EventArgs e) {
            try {
                ...
                jsActionCancel = string.Format("rbnCancelClick('{0}/{1}'); return false;", SPContext.Current.Web.Url, Pages.ServerFunctions.PAGE_URL);
                itemRedirect = string.Format("{0}/{1}", SPContext.Current.Web.Url, Pages.ServerFunctions.PAGE_URL);
                newItemRedirect = string.Format("{0}/{1}?View=New&id={2}", SPContext.Current.Web.Url, Pages.ServerFunctionItem.PAGE_URL, 0);
        ...
        public virtual string GetItemsURL(bool AsLink) {
            ...
            try {
                ...
                s = (AsLink) ? string.Format("<a href=\"{0}\" class=\"mapps-breadcrumb\">Server Functions</a>", itemRedirect) : itemRedirect;
            } catch (Exception ex) {
                MAPPS.Error.WriteError(ex);
            }
            return s;
        ...
        private void SetupContribute() {
            ...
            if (IsManager) {
                item = (ItemID == 0) ? new MAPPS.ServerFunction() : new MAPPS.ServerFunction(ItemID);
            } else {
        ...
        private bool SaveItem() {
            ...
            try {
                ...
                item = (isUpdate) ? new MAPPS.ServerFunction(ItemID) : new MAPPS.ServerFunction();
        ...
        private bool DeleteItem() {
            bool success = false;
            try {
                MAPPS.ServerFunction item = new MAPPS.ServerFunction(ItemID);
                if (item.Delete()) {
                    success = true;
                    ItemID = 0;
                } else {
        ...
        protected void lbtnEdit_Click(object sender, EventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=Edit&id={2}", SPContext.Current.Web.Url, Pages.ServerFunctionItem.PAGE_URL, ItemID), false);
        }

        protected void lbtnView_Click(object sender, EventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=View&id={2}", SPContext.Current.Web.Url, Pages.ServerFunctionItem.PAGE_URL, ItemID), false);
        }
