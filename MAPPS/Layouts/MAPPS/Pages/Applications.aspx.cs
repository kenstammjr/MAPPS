...
namespace MAPPS.Pages {
    public partial class Applications : PageBase {
        protected const string PAGE_TITLE = "Applications";
        protected const string PAGE_DESCRIPTION = "View and manage applications";
        public const string PAGE_URL = BASE_PAGES_URL + "Applications.aspx";
        ...
        protected override void Page_Load(object sender, EventArgs e) {
            try {
                ...
                SetupContribute();
                if (!IsPostBack) {
                    GridSortExpression = "DisplayIndex, Name";
                    Fill();
                }
            } catch (Exception ex) {
        ...
        private void SetupContribute() {
            if (IsManager) {
                jsActionNewItem = string.Format("window.location.replace('{0}/{1}?View=New&ID=0'); return false;", SPContext.Current.Web.Url, ApplicationItem.PAGE_URL);
        ...
        protected override void Fill() {
            try {
                ...
                DataView dv = new DataView(MAPPS.Application.Items().Tables[0]);
            ...
            try {
                switch (e.Row.RowType) {
                    case DataControlRowType.DataRow:
                        ...
                        Label lblProtected = (Label)e.Row.FindControl("lblProtected");
                        ImageButton ibtnEdit = (ImageButton)e.Row.FindControl("ibtnEdit");
                        ibtnEdit.Visible = lblProtected.Text == "True" ? false : true;
                        ...
                        lblProtected.Text = lblProtected.Text == "True" ? "<img src=\"/_layouts/15/images/kpidefault-0.gif\" />" : "<img src=\"/_layouts/15/images/kpidefault-2.gif\" />";
                        break;
            ...
            if (e.CommandName != "Sort" && e.CommandName != "Page") {
                try {
                    if (e.CommandName == "ViewItem") {
                        ...
                        Response.Redirect(string.Format("{0}/{1}?View=View&ID={2}", SPContext.Current.Web.Url, ApplicationItem.PAGE_URL, ItemID), false);
                    }
                    if (e.CommandName == "EditItem") {
                        ItemID = int.Parse(e.CommandArgument.ToString());
                        Response.Redirect(string.Format("{0}/{1}?View=Edit&ID={2}", SPContext.Current.Web.Url, ApplicationItem.PAGE_URL, ItemID), false);
                    }
                    // Move Down
                    if (e.CommandName == "MoveDown") {
                        ItemID = int.Parse(e.CommandArgument.ToString());
                        MAPPS.Application.UpdateDisplayIndex(ItemID, "up");
                        Fill();
                    }
                    // Move Up
                    if (e.CommandName == "MoveUp") {
                        ItemID = int.Parse(e.CommandArgument.ToString());
                        MAPPS.Application.UpdateDisplayIndex(ItemID, "down");
                        Fill();
                    }
                } catch (Exception ex) {
                    MAPPS.Error.WriteError(ex);
        ...
        protected void lbtnNew_Click(object sender, EventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=New&id={2}", SPContext.Current.Web.Url, Pages.ApplicationItem.PAGE_URL, 0), false);
        }
