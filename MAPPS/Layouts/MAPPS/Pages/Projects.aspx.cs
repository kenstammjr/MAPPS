...
namespace MAPPS.Pages {
    public partial class Projects : PageBase {
        protected const string PAGE_TITLE = "Projects";
        protected const string PAGE_DESCRIPTION = "View and manage projects";
        public const string PAGE_URL = BASE_PAGES_URL + "Projects.aspx";
        ...
        private void SetupContribute() {
            if (IsManager) {
                ...
                jsActionNewItem = string.Format("openModalDialog('Project - New Item','{0}/{1}?View=New&ID=0&UserID=0&Filter={2}'); return false;", SPContext.Current.Web.Url, Pages.ProjectItem.PAGE_URL, Filter);
        ...
        protected override void Fill() {
            try {
                ...
                if (txtSearch.Text == string.Empty) {
                    ...
                    dv = new DataView(MAPPS.Project.Items().Tables[0]);
                } else {
                    gvData.EmptyDataText = Message.EMPTY_LIST_SEARCHED;
                    dv = new DataView(MAPPS.Project.Items(txtSearch.Text.Trim()).Tables[0]);
                }
            ...
            try {
                switch (e.Row.RowType) {
                    case DataControlRowType.DataRow:
                        ...
                        string urlView = string.Format("{0}/{1}?View=View&ID={2}&PortID={2}&Filter={3}", this.Web.Url, Pages.ProjectItem.PAGE_URL, id, txtSearch.Text.Trim());
                        string urlEdit = string.Format("{0}/{1}?View=Edit&ID={2}&PortID={2}&Filter={3}", this.Web.Url, Pages.ProjectItem.PAGE_URL, id, txtSearch.Text.Trim());

                        ibtnEdit.OnClientClick = "openModalDialog('Project - " + string.Format("{0}", lbtnName.Text) + "', '" + urlEdit + "'); return false;";
                        lbtnName.OnClientClick = "openModalDialog('Project - " + string.Format("{0}", lbtnName.Text) + "', '" + urlView + "'); return false;";
            ...
            if (e.CommandName != "Sort" && e.CommandName != "Page") {
                try {
                    if (e.CommandName == "ViewItem") {
                        ...
                        Response.Redirect(string.Format("{0}/{1}?View=View&ID={2}&Filter={3}", SPContext.Current.Web.Url, ProjectItem.PAGE_URL, ItemID, Filter), false);
                    }
                    if (e.CommandName == "EditItem") {
                        ItemID = int.Parse(e.CommandArgument.ToString());
                        Response.Redirect(string.Format("{0}/{1}?View=Edit&ID={2}&Filter={3}", SPContext.Current.Web.Url, ProjectItem.PAGE_URL, ItemID, Filter), false);
                    }
                } catch (Exception ex) {
                    MAPPS.Error.WriteError(ex);
                    if (ShowDebug)
        ...
        protected void lbtnNew_Click(object sender, EventArgs e) {
            Response.Redirect(string.Format("{0}/{1}?View=New&id={2}&Filter={3}", SPContext.Current.Web.Url, Pages.ProjectItem.PAGE_URL, 0, Filter), false);
        }
