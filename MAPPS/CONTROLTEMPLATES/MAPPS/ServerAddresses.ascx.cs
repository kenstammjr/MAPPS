...
namespace MAPPS.CONTROLTEMPLATES {
    public partial class ServerAddresses : ServerControlBase {
        ...
        protected override void Fill() {
            try {
                ...
                if (Request["addaddress"] != null) {
                    hfItemID.Value = "0";
                    btnDelete.Visible = false;
                    FillItem("Edit", 0);
                }

                DataView dv = new DataView(ServerAddress.Items(ServerID).Tables[0]);
                gvData.EmptyDataText = "No addresses to display";
        ...
        private void FillItem(string Mode, int ID) {
            ...
            ServerAddress item = new ServerAddress(ID);
            try {
        ...
        private void SaveItem() {
            ...
            try {
                if (PassedValidation()) {
                    ServerAddress item = new ServerAddress(int.Parse(hfItemID.Value));
                    MAPPS.User user = new MAPPS.User(Context.User.Identity.Name);
